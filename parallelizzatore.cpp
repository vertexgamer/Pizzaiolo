...
int main()
{
    ...
    cout << "Scrivi gli argomenti ma sostituisci con il nome \"percorso\" il percorso di input e output / Write the arguments but replace the input and output path with the word \"percorso\"" << endl; //the user write the desired arguments but replace the input and output path with "percorso"
    ...
    int i = eseguibile.find_last_of("\\");          //the executable name is extracted from the path given before
    ...
    for (const auto& entry : fs::directory_iterator(cartella)) {    //it gets the path of the files inside the folder
        ...
        while (argomenti.find("percorso") != string::npos)
            argomenti.replace(argomenti.find("percorso"), 8, entry.path().string());    //it replace "percorso" with the file path
